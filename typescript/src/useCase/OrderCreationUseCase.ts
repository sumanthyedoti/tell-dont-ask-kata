...
class OrderCreationUseCase {
  ...
  public run(request: SellItemsRequest): void {
    ...
    for (const itemRequest of request.getRequests()) {
      const product: Product = this.productCatalog.getByName(
      ...
      if (product === undefined) {
        ...
        const unitaryTaxedAmount: number =
          Math.round((product.getPrice() + unitaryTax) * 100) / 100;
        const taxedAmount: number =
          Math.round(unitaryTaxedAmount * itemRequest.getQuantity() * 100) /
          100;
